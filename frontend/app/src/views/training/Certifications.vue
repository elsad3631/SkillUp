<template>
  <div class="card hover-card">
    <div class="card-header border-0 pt-6">
      ...
      <div class="card-toolbar">
        <button @click="openCreateCertificationModal()" class="btn btn-primary modern-btn">
  ...
  <CreateCertificationModal
    :is-editing="!!editingCertification"
    :certification="editingCertification"
    :loading="saving"
    :users="users"
    :close-modal="closeCertificationModal"
...
export default defineComponent({
  ...
  setup() {
    ...
    const openCreateCertificationModal = () => {
      editingCertification.value = null; // Ensure we're in create mode
      ...
      if (!certificationModal) {
        ...
        if (modalElement) {
          certificationModal = new Modal(modalElement);
        }
      }
    ...
    const handleCertificationSubmit = async (formData: any) => {
      saving.value = true;
      try {
        const submitData = {
          ...formData,
        ...
        if (editingCertification.value) {
          const result = await updateCertification(editingCertification.value.id, submitData);
          if (result) {
